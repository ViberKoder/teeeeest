...
describe('metadataUriStale', () => {
  const base = 'https://example.com/jetton-metadata5.json';
  ...
  it('metadataUriPathname strips query', () => {
    expect(metadataUriPathname(bumped)).toBe('https://example.com/jetton-metadata5.json');
  });

  it('stale when on-chain ?v= differs from cached', () => {
    expect(
      metadataUriStale(
        'https://example.com/jetton-metadata5.json?v=2',
        'https://example.com/jetton-metadata5.json?v=3',
        base,
      ),
    ).toBe(true);
  });
...
describe('epochMetadataUri', () => {
  it('sets v query to epoch', () => {
    expect(epochMetadataUri('https://example.com/jetton-metadata5.json', 30)).toBe(
      'https://example.com/jetton-metadata5.json?v=30',
    );
  });
});
...
describe('bumpMetadataUri', () => {
  it('increments v query param', () => {
    expect(bumpMetadataUri('https://example.com/jetton-metadata5.json?v=2')).toBe(
      'https://example.com/jetton-metadata5.json?v=3',
    );
  });
});
