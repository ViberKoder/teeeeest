...
`https://<ваш-хост>/api/v1/jettons/EQ…master/metadata.json` (канонический URL; минтер подставляет master при деплое)
...
## Если метаданные не на RMJ-бэкенде

В мастере выберите режим «Свой URL JSON», залейте JSON на Gist/S3. В файле обязательно:
`custom_payload_api_uri`: `https://<backend>/api/v1/jettons/<EQ master>` (без `/custom-payload` и без trailing `/`).

> **⚠️ Критично:** используйте JSON, который генерирует мастер (кнопка «Скачать JSON»). Файл `contracts/jetton-metadata-example.json` из репозитория — шаблон-пример; если загрузить его без изменений, поле `custom_payload_api_uri` будет содержать плейсхолдер, кошельки не смогут найти невостребованные токены и остатки не отобразятся.
