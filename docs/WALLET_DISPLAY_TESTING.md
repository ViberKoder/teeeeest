...
|-----|----------------|----------|
| On-chain URI | `curl 'https://toncenter.com/api/v2/getTokenData?address=EQ…MASTER'` → `jetton_content.data` | `https://…/jetton-metadata2.json` |
...
2. **Бэкенд:** задеплойте ветку с фиксированным `…/jetton-metadata2.json` и выставьте  
   `JETTON_MASTER_ADDRESS=EQ…ваш_master`.
3. **Контракт (admin):** `change_content` (op `4`) → off-chain URI  
   `https://YOUR-BACKEND/jetton-metadata2.json` (смена URL сбрасывает кэш TonAPI).
4. **Проверка:**
   ```bash
   curl -sSL 'https://YOUR-BACKEND/jetton-metadata2.json' | jq .custom_payload_api_uri
