...
2. **Бэкенд:** задеплойте ветку с фиксированным `…/jetton-metadata.json` и выставьте  
   `JETTON_MASTER_ADDRESS=EQB5eNZTh5T3ZrWyx-02WpIslZlv9kYdDDM1KPxyU3bR0OSD`.
3. **Контракт (admin):** `change_content` (op `4`) → off-chain URI только  
   `https://teeeeest-production.up.railway.app/jetton-metadata.json` (без `/jettons/…` в пути).
4. **Проверка:**
   ```bash
   curl -sSL 'https://teeeeest-production.up.railway.app/jetton-metadata.json' | jq .custom_payload_api_uri
...
Proof API отдаёт **TEP-177** payload: op `0x0df602d6` + merkle proof (как ton-community/mintless-jetton).
Контракт wallet принимает и `0x0df602d6`, и legacy RMJ `0xc9e56df3` (voucher + proof).
